...
int wmain(const int argc, wchar_t** argv) {
	...
	if (drvIndex > 0) {
		...
		raw_image = embedded_driver::GetEmbeddedDriver();
		
		if (raw_image.empty()) {
			kdmLog(L"[-] PUT YOUR DRIVER IN HEX.HPP" << std::endl);
			help();
			intel_driver::Unload();
			PauseIfParentIsExplorer();
			return -1;
	...
	system("cls");
	kdmLog(L"[+] Driver has Mapped successfully, You can now Close the program." << std::endl);
	Sleep(10000);
